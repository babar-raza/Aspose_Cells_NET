...
Namespace Aspose.Cells.Demos
	Partial Public Class FinancialPlan
		...
		Protected Sub btnProcess_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Dim path As String = MapPath("~/designer/FinancialPlan.xls")
		...
		Private Sub AddStyles(ByVal workbook As Workbook)
			...
			For i As Integer = 0 To 27
				...
				'Name style
				style.Name = "Custom_Style" & (CInt(Fix(i + 1))).ToString()
		...
		Private Sub FillModelInputs(ByVal workbook As Workbook)
			'Get a worksheet named "Model Inputs"
			Dim sheet As Worksheet = workbook.Worksheets("Model Inputs")
			...
			Dim range As Range = cells.CreateRange("C24", "F24")
			Dim iArray() As Integer = { 2800, 1200, 1600, 0 }
			...
			iArray = New Integer() { 120, 80, 40, 0 }
			cells.ImportArray(iArray, range.FirstRow, range.FirstColumn, False)
			...
			Dim strArray() As String = { "=+C24*C25", "=+D24*D25", "=+E24*E25", "=+F24*F25" }
			...
			Dim dArray() As Double = { 0.5, 0.4, 0.25, 0 }
		...
		Private Sub FillProfitLoss(ByVal workbook As Workbook)
			'Get the worksheet named "Profit and Loss"
			Dim sheet As Worksheet = workbook.Worksheets("Profit and Loss")
			...
			Dim obj2DArray(,) As Object = {{"-",0.02,0.04,0.06,0.08}, {"-",0.02,0.04,0.06,0.08}, {0.005,0.005,0.005,0.005,0.005}}
			'Create range from cells E8 to I10
			Dim range As Range = cells.CreateRange("E8", "I10")
			...
			Dim strArray() As String = { "=+'Model Inputs'!C28", "=+E15*(1+F8)", "=+F15*(1+G8)", "=+G15*(1+H8)", "=+H15*(1+I8)" }
			...
			strArray = New String() { "=('Balance Sheet'!D15+'Balance Sheet'!D17+'Balance Sheet'!D18)/'Model Inputs'!C40", "=('Balance Sheet'!E15+'Balance Sheet'!E17+'Balance Sheet'!E18)/'Model Inputs'!C40*(1+F9)", "=('Balance Sheet'!F15+'Balance Sheet'!F17+'Balance Sheet'!F18)/'Model Inputs'!C40*(1+G9)", "=('Balance Sheet'!G15+'Balance Sheet'!G17+'Balance Sheet'!G18)/'Model Inputs'!C40*(1+H9)", "=('Balance Sheet'!H15+'Balance Sheet'!H17+'Balance Sheet'!H18)/'Model Inputs'!C40*(1+I9)"}
			...
			strArray = New String() { "=+'Balance Sheet'!D18*'Model Inputs'!C38", "=('Balance Sheet'!E18*'Model Inputs'!C38)*(1+F9)", "=('Balance Sheet'!F18*'Model Inputs'!C38)*(1+G9)", "=('Balance Sheet'!G18*'Model Inputs'!C38)*(1+H9)", "=('Balance Sheet'!H18*'Model Inputs'!C38)*(1+I9)"}
			...
			strArray = New String() { "=-SUM('Loan Payment Calculator'!F12:F23)","=-SUM('Loan Payment Calculator'!F24:F35)", "=-SUM('Loan Payment Calculator'!F36:F47)","=-SUM('Loan Payment Calculator'!F48:F59)", "=-SUM('Loan Payment Calculator'!F60:F71)"}
			...
			strArray = New String() { "=IF(E44<0,0,D46*E44)","=IF(F44<0,0,D46*F44)", "=IF(G44<0,0,D46*G44)","=IF(H44<0,0,D46*H44)", "=IF(I44<0,0,D46*I44)"}
		...
		Private Sub FillBalanceSheet(ByVal workbook As Workbook)
			'Get the worksheet "Balance Sheet"
			Dim sheet As Worksheet = workbook.Worksheets("Balance Sheet")
			...
			'E7 to I7
			Dim strArray() As String = { "=+'Cash Flow'!D36", "=+'Cash Flow'!E36", "=+'Cash Flow'!F36", "=+'Cash Flow'!G36", "=+'Cash Flow'!H36" }
			Dim range As Range = cells.CreateRange("E7", "I7")
			cells.ImportFormulaArray(strArray, range.FirstRow, range.FirstColumn, False)
			range.ApplyStyle(workbook.Styles("Custom_Style10"), styleflag)
			range.SetOutlineBorder(BorderType.RightBorder, CellBorderType.Medium, Color.Black)

			'D8 to D12
			Dim iArray() As Integer = { 3000, 25000, 0, 0, 5000 }
			...
			'D16 to D19
			iArray = New Integer() { 100000, 0, 100000, 0 }
			...
			'D38 to I38
			iArray = New Integer() { 100000, 200000, 150000, 175000, 225000, 150000 }
		...
		Private Sub FillCashFlow(ByVal workbook As Workbook)
			'Get the worksheet named  "Cash Flow"
			Dim sheet As Worksheet = workbook.Worksheets("Cash Flow")
			...
			'D8 to I8
			Dim strArray() As String = { "=+'Profit and Loss'!E48", "=+'Profit and Loss'!F48", "=+'Profit and Loss'!G48", "=+'Profit and Loss'!H48", "=+'Profit and Loss'!I48", "=SUM(D8:H8)" }
			Dim range As Range = cells.CreateRange("D8", "I8")
			cells.ImportFormulaArray(strArray, range.FirstRow, range.FirstColumn, False)
			range.ApplyStyle(workbook.Styles("Custom_Style10"), styleflag)
			range.SetOutlineBorder(BorderType.RightBorder, CellBorderType.Medium, Color.Black)
		...
		Private Sub FillLoanPaymentCalculator(ByVal workbook As Workbook)
			'Get the worksheet named "Loan Payment Calculator"
			Dim sheet As Worksheet = workbook.Worksheets("Loan Payment Calculator")
			...
			Dim range As Range = cells.CreateRange("C5", "C9")
			range.SetOutlineBorder(BorderType.TopBorder, CellBorderType.Thin, Color.Black)
